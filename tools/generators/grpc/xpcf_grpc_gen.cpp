...
void parse_entity(const cppast::cpp_entity_index& idx, std::ostream& out, const cppast::cpp_entity& e)
{
    ...
    for (auto & c : interfaces) {
        ...
        xpcf::getComponentManagerInstance()->resolve<IRPCGenerator>()->generate(c);
    }
}
...
try
{
    ...
    option_list.add_options("generation")
            ("d,destination", "set the destination folder where the generated grpc files will be created. The folder is created if it doesn't already exists",
             cxxopts::value<std::string>());
    ...
    else if (options.count("version"))
    {
        ...
        if (options.count("destination")) {
            ...
            rpcGenerator->setDestinationFolder(options["destination"].as<std::string>());
        }
